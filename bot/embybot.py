...
def renshu_gyf():
    nowplay = len([r['PlayState']['CanSeek'] for r in requests.get('http://10.10.10.122:8096/Sessions?api_key=db0cdfb001124a5ab5d1da47377c03a3').json()if r['PlayState']['CanSeek'] == True])
    return nowplay
...
async def my_handler(client, message):
    ...
    elif text == '/status' or text == f'/status{bot_name}':
        ws = create_connection("wss://tz.091321.xyz/ws", timeout=10, sslopt={"cert_reqs": ssl.CERT_NONE}) # 此处对照修改自己探针的访问地址
        if ws.connected:
            ws.send('8')
            res = ws.recv()
            resjson = json.loads(res)
            gyf_CPU = resjson["servers"][15]["Host"]["CPU"][0] # 此处以及下面的resjson["servers"][0]中的[0]对照哪吒探针中的机器所属第几位进行修改，起始数字为0
            gyf_currentCPU = round(resjson["servers"][15]["State"]["CPU"], 2)  # %
            gyf_memtoal = round(resjson["servers"][15]["Host"]["MemTotal"] / 1024 / 1024 / 1024, 2)  # G
            gyf_currentMem = round(resjson["servers"][15]["State"]["MemUsed"] / 1024 / 1024 / 1024, 2)  # G
            gyf_netin = round(resjson["servers"][15]["State"]["NetInSpeed"] / 1024 / 100, 2)  # Mbps
            gyf_netout = round(resjson["servers"][15]["State"]["NetOutSpeed"] / 1024 / 100, 2)  # Mbps
            gyf_load = round(resjson["servers"][15]["State"]["Load1"] / 12 * 100, 2)  # %
            ...
            message_gyf = t + "\n\n" + "`Mildnes_Emby状态播报`\n" + " - 当前观看人数：" + str(
                ...
                gyf_currentMem) + "GB\\" + str(gyf_memtoal) + "GB\n - 实时上传速率：" + str(gyf_netout) + "Mbps\n - 实时下载速率：" + str(
                gyf_netin) + "Mbps\n - 服务器实时负载：" + str(gyf_load) + "%"
            await app.send_message(chat_id=message.from_user.id,text=message_gyf)
        else:
            message_gyf = "获取服务器状态失败，请通知管理员检查。"
            await app.send_message(chat_id=message.from_user.id,text=message_gyf)
